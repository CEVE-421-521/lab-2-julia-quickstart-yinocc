...
function read_tides(year::Int)
    ...
    df[!, "Date Time"] = DateTime.(df[!, "Date Time"], date_format)
    # 3. convert the " Water Level" column to meters
    df[!, " Water Level"] .*= 1u"m"
    # 4. rename the columns to "datetime" and "lsl"
    rename!(df, "Date Time" => "datetime", " Water Level" => "lsl")
    # 5. select the "datetime" and "lsl" columns
    selected_df = select(df, :datetime, :lsl)
...
t_end = Dates.DateTime(2020, 4, 1, 0)
